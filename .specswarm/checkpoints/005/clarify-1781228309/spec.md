...
2. In the Ingredients tab, each **primary** ingredient (the one being substituted) shows an
   inline annotation listing what can replace it — e.g. "or: Almond flour".
3. Ingredients with no substitutes pointing at them show nothing extra.
...
- On `/recipes/[id]`, within the Ingredients tab, the annotation appears on the **primary**
  ingredient row (the one being substituted), not on the substitute row.
- For each ingredient, find all other ingredients whose `substituteFor` equals this ingredient's
  `id` — those are its substitutes. If any exist, render an inline annotation listing their names.
- The annotation MUST be visually subordinate to the primary ingredient name (smaller text, muted
  colour). Pattern: "or: [name]" (single substitute) or "or: [name], [name]" (multiple).
- When no substitutes point at an ingredient, the annotation MUST NOT be rendered (no empty placeholder).
- The lookup MUST use `recipe.ingredients` (the full `RecipeIngredient[]` array, which carries
  the `substituteFor` UUID) — not `scaledIngredients`, which may not expose that field.
...
- Q: On the detail page, which row carries the substitute annotation? → A: The primary ingredient row (reverse lookup — find all ingredients pointing at this one). Pattern: "or: [name]".
- Q: Should multi-level substitute chains be restricted in the picker? → A: No constraint added; picker shows all siblings except self. Existing INV-RC-011 is the only enforced rule.
